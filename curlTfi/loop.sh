...
echo "size_header size_request time_namelookup time_connect time_appconnect time_pretransfer time_redirect time_starttransfer time_total" > resHttp20.csv
echo "size_header size_request time_namelookup time_connect time_appconnect time_pretransfer time_redirect time_starttransfer time_total" > resHttps20.csv
echo "done!"

echo "curl loop 10 ..."
for ((i=1;i<=10;i++)); 
do
curl -w "@curl-format.txt" -o /dev/null -s https://tfium.eastus.cloudapp.azure.com:8000 >> resHttp10.csv;
...
for ((i=1;i<=20;i++)); 
do 
curl -w "@curl-format.txt" -o /dev/null -s https://tfium.eastus.cloudapp.azure.com:8000 >> resHttp20.csv;
curl -w "@curl-format.txt" -o /dev/null -s https://tfium.eastus.cloudapp.azure.com >> resHttps20.csv;
done

echo "curl loop 100 ..."
for ((i=1;i<=100;i++)); 
do
curl -w "@curl-format.txt" -o /dev/null -s https://tfium.eastus.cloudapp.azure.com:8000 >> resHttp100.csv;
